  ...
  return (
    <main style={{ background: '#0a0a0f', minHeight: '100vh', color: '#fff', fontFamily: 'system-ui, sans-serif', overflowX: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', zIndex: 1000, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px' }}>
        {showChat && (
          <div style={{ background: '#13131a', border: '1px solid rgba(168,85,247,0.4)', borderRadius: '20px', padding: '1.5rem', width: '300px', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              ...
              <button onClick={() => setShowChat(false)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: '18px', lineHeight: '1' }}>✕</button>
            </div>
        ...
        <button onClick={() => setShowChat(!showChat)} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'linear-gradient(135deg, #a855f7, #7c3aed)', border: 'none', borderRadius: '100px', padding: '14px 22px', color: '#fff', cursor: 'pointer', fontSize: '15px', fontWeight: '700', boxShadow: '0 0 40px rgba(168,85,247,0.5)' }}>
          <span style={{ fontSize: '20px' }}>{showChat ? '✕' : '💬'}</span>
          <span>{showChat ? t.close : t.support_title}</span>
        </button>
      </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '8rem 2rem 4rem' }}>
        ...
        <div style={{ marginTop: '5rem', width: '100%', maxWidth: '800px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(168,85,247,0.2)', borderRadius: '20px', padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textAlign: 'left' }}>
            <div style={{ alignSelf: 'flex-end', background: 'linear-gradient(135deg, #a855f7, #7c3aed)', borderRadius: '14px 14px 4px 14px', padding: '12px 18px', maxWidth: '70%', fontSize: '14px' }}>{t.chat_msg}</div>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: '4px 14px 14px 14px', padding: '12px 18px', maxWidth: '70%', fontSize: '14px', color: 'rgba(255,255,255,0.8)' }}>{t.chat_reply}</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
          {t.plans.map((plan, i) => (
            <div key={i} style={{ background: plan.accent ? 'linear-gradient(135deg, rgba(168,85,247,0.2), rgba(124,58,237,0.2))' : 'rgba(255,255,255,0.03)', border: plan.accent ? '1px solid rgba(168,85,247,0.5)' : '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', padding: '2.5rem 2rem', position: 'relative', transform: plan.accent ? 'scale(1.05)' : 'none', boxShadow: plan.accent ? '0 0 60px rgba(168,85,247,0.2)' : 'none' }}>
              ...
              <a href="/register" style={{ display: 'block', width: '100%', background: plan.accent ? 'linear-gradient(135deg, #a855f7, #7c3aed)' : 'rgba(255,255,255,0.08)', border: plan.accent ? 'none' : '1px solid rgba(255,255,255,0.15)', borderRadius: '12px', padding: '14px', color: '#fff', fontSize: '15px', fontWeight: '700', textDecoration: 'none', textAlign: 'center' }}>
                {plan.btn}
              </a>
            </div>
          ))}
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '6rem 2rem', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto', background: 'linear-gradient(135deg, rgba(168,85,247,0.15), rgba(251,146,60,0.1))', border: '1px solid rgba(168,85,247,0.3)', borderRadius: '32px', padding: '5rem 3rem' }}>
          ...
          <a href="/register" style={{ display: 'inline-block', background: 'linear-gradient(135deg, #a855f7, #7c3aed)', borderRadius: '14px', padding: '18px 48px', color: '#fff', fontSize: '18px', fontWeight: '700', boxShadow: '0 0 60px rgba(168,85,247,0.5)', textDecoration: 'none' }}>
            {t.cta_btn} 🚀
          </a>
        </div>
      </section>
      ...
      {/* Соцсети */}
      <section style={{ position: 'relative', zIndex: 1, padding: '5rem 2rem', textAlign: 'center', borderTop: '1px solid rgba(168,85,247,0.15)', borderBottom: '1px solid rgba(168,85,247,0.15)' }}>
        <h3 style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '0.75rem', color: '#fff' }}>{t.social_title}</h3>
        <p style={{ color: 'rgba(255,255,255,0.5)', marginBottom: '2.5rem', fontSize: '1rem' }}>{t.social_sub}</p>
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap', maxWidth: '800px', margin: '0 auto' }}>
          <a href={t.tg_channel} target="_blank" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(0,136,204,0.15)', border: '1px solid rgba(0,136,204,0.4)', borderRadius: '14px', padding: '14px 24px', color: '#fff', textDecoration: 'none', fontSize: '15px', fontWeight: '600' }}>
            ✈️ {t.tg_channel_name}
          </a>
          <a href="https://t.me/growlymanager" target="_blank" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(0,136,204,0.15)', border: '1px solid rgba(0,136,204,0.4)', borderRadius: '14px', padding: '14px 24px', color: '#fff', textDecoration: 'none', fontSize: '15px', fontWeight: '600' }}>
            ✈️ {lang === 'ru' ? 'Менеджер' : 'Manager'}
          </a>
          <a href="https://www.instagram.com/growlyrazvitie?igsh=MTdjOXc4M3dxNW41Nw%3D%3D&utm_source=qr" target="_blank" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(225,48,108,0.15)', border: '1px solid rgba(225,48,108,0.4)', borderRadius: '14px', padding: '14px 24px', color: '#fff', textDecoration: 'none', fontSize: '15px', fontWeight: '600' }}>
            📸 Instagram
          </a>
          <a href="https://www.tiktok.com/@growly27" target="_blank" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '14px', padding: '14px 24px', color: '#fff', textDecoration: 'none', fontSize: '15px', fontWeight: '600' }}>
            🎵 TikTok
          </a>
          <a href="https://youtube.com/@growlyrazvitie?si=FhB9yCkcHnrT4TxI" target="_blank" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(255,0,0,0.15)', border: '1px solid rgba(255,0,0,0.4)', borderRadius: '14px', padding: '14px 24px', color: '#fff', textDecoration: 'none', fontSize: '15px', fontWeight: '600' }}>
            ▶️ YouTube
          </a>
        </div>
      </section>
      ...
      {/* Footer */}
      <footer style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '2rem', color: 'rgba(255,255,255,0.3)', fontSize: '14px' }}>
        {t.footer}
      </footer>
